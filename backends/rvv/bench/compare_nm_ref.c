...
static void ref_nm(
    ...
    sattn_shape_t s, int n, int m) {
  // Map N:M to block_topk reference: block_size=m, keep_ratio=n/m, no globals
  int block = m > 0 ? m : 64;
  float keep = (n > 0 && m > 0) ? ((float)n / (float)m) : 0.25f;
  // reuse the block_topk CPU reference from compare_blocktopk_ref.c (inline minimal)
  const int64_t B = s.B, H = s.H, L = s.L, D = s.D;
  const int64_t num_blocks = (L + block - 1) / block;
  int k_blocks = (int)((float)num_blocks * keep + 0.999f); if (k_blocks < 1) k_blocks = 1;
  ...
  int* sel_idx = (int*)malloc((size_t)(k_blocks * block) * sizeof(int));
  for (int64_t b = 0; b < B; ++b) for (int64_t h = 0; h < H; ++h) {
    for (int64_t i = 0; i < L; ++i) {
      ...
      int sel_cnt = 0; int sel_cap = k_blocks * block;
...
int main() {
  ...
  ref_nm(Q,K,V,Oref,s,p.n,p.m);
  sattn_rvv_nm_structured(Q,K,V,Orvv,s,p);
