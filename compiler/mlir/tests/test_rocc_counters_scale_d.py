...
def run_mlir_and_capture(mlir_src):
    p = 'compiler/mlir/tests/_tmp_scale_d.mlir'
...
def test_counters_scale_with_D():
    S = 64
    base = run_mlir_and_capture(f'module {{\n  "sattn.sparse_attention"() {{ tile_M = 16 : i64, tile_D = 16 : i64, tile_S = {S} : i64 }} : () -> ()\n}}\n')
    big  = run_mlir_and_capture(f'module {{\n  "sattn.sparse_attention"() {{ tile_M = 16 : i64, tile_D = 32 : i64, tile_S = {S} : i64 }} : () -> ()\n}}\n')
